			...
			grub_uint32_t bits)
{
  grub_xhci_write32(&x->pr[port].portsc,
      (grub_xhci_read32(&x->pr[port].portsc) &
       ~(GRUB_XHCI_PORTSC_PED|GRUB_XHCI_PORTSC_PR|GRUB_XHCI_PORTSC_CSC))
      & ~(bits));
}
...
static void xhci_process_events(struct grub_xhci *x)
{
    ...
    for (;;) {
        ...
        {
            grub_dprintf("xhci", "%s: portstatus changed (cc %d)\n",
                __func__, evt_cc);
            break;
        }
        default:
			  ...
			  unsigned int port, unsigned int enable)
{
 ...
 grub_dprintf("xhci", "grub_xhci_portstatus port #%d: 0x%08x,%s%s pls %d enable %d\n",
  ...
  if (!enable) {
    // Disable port
    grub_xhci_port_setbits(x, port, GRUB_XHCI_PORTSC_PED);
    return GRUB_USB_ERR_NONE;
  }
  ...
  if (portsc & GRUB_XHCI_PORTSC_CSC)
    {
      *changed = 1;
      ...
      grub_dprintf("debug", " STATUS CHANGED: %s : #%d: 0x%08x pls %d\n",
          (portsc & GRUB_XHCI_PORTSC_CCS)  ? "ENABLED" : "DISABLED",
          port, portsc,
          pls);
